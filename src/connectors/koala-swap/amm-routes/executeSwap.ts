...
import { KoalaSwapAmmExecuteSwapRequest } from '../schemas';

import { getKoalaSwapAmmQuote } from './quoteSwap';
...
export async function executeAmmSwap(
  ...
  // Find pool address
  const poolAddress = await koalaSwap.findDefaultPool(baseToken, quoteToken, 'amm');
  if (!poolAddress) {
  ...
  const { quote } = await getKoalaSwapAmmQuote(
    fastify,
    network,
    poolAddress,
    baseToken,
  ...
  const routerAddress = getKoalaSwapV2RouterAddress(network);
  ...
  if (currentAllowance.lt(amountNeeded)) {
    ...
    throw fastify.httpErrors.badRequest(
      `Insufficient allowance for ${quote.inputToken.symbol}. Please approve at least ${formatTokenAmount(amountNeeded, quote.inputToken.decimals)} ${quote.inputToken.symbol} for the KoalaSwap router (${routerAddress})`,
    );
  }
  ...
  try {
    if (isHardwareWallet) {
      ...
      const iface = new utils.Interface(IKoalaSwapV2Router02ABI.abi);
      let data;
      ...
      const routerContract = new Contract(routerAddress, IKoalaSwapV2Router02ABI.abi, wallet);
...
export const executeSwapRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    {
      schema: {
        description: 'Execute a swap on KoalaSwap V2 AMM using Router02',
        tags: ['/connector/koala-swap'],
        body: KoalaSwapAmmExecuteSwapRequest,
    ...
    async (request) => {
      try {
        ...
        const {
          ...
          side = 'SELL',
          slippagePct = 1,
        } = request.body as typeof KoalaSwapAmmExecuteSwapRequest._type;
