...
import { IKoalaSwapV2PairABI } from '../koala-swap.contracts';
import { formatTokenAmount } from '../koala-swap.utils';
import { KoalaSwapAmmGetPoolInfoRequest } from '../schemas';
...
export const poolInfoRoute: FastifyPluginAsync = async (fastify) => {
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get AMM pool information from KoalaSwap V2',
        tags: ['/connector/koala-swap'],
        querystring: KoalaSwapAmmGetPoolInfoRequest,
        response: {
          200: PoolInfoSchema,
        },
      },
    ...
    async (request): Promise<PoolInfo> => {
      try {
        ...
        // For KoalaSwap, we need to get the pair contract to extract token addresses
        ...
        const token0 = koalaSwap.getTokenByAddress(token0Address);
        const token1 = koalaSwap.getTokenByAddress(token1Address);
        ...
        // Get V2 pair data
        const v2Pair = await koalaSwap.getV2Pool(token0, token1, poolAddress);
        ...
        return {
          ...
          feePct: 0.3, // KoalaSwap V2 fee is fixed at 0.3%
          price: price,
