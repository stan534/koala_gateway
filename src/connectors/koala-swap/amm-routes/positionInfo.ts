...
export async function checkLPAllowance(
  ...
  if (currentLpAllowance.lt(requiredAmount)) {
    throw new Error(
      `Insufficient LP token allowance. Please approve at least ${formatTokenAmount(requiredAmount.toString(), 18)} LP tokens (${poolAddress}) for the KoalaSwap router (${routerAddress})`,
    );
  }
}
...
export const positionInfoRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get position information for a KoalaSwap V2 pool',
    ...
    async (request) => {
      try {
        ...
        const baseTokenObj = koalaSwap.getTokenByAddress(token0);
        const quoteTokenObj = koalaSwap.getTokenByAddress(token1);
