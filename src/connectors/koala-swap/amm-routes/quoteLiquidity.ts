...
import { IKoalaSwapV2PairABI, getKoalaSwapV2RouterAddress } from '../koala-swap.contracts';
import { formatTokenAmount, getKoalaSwapPoolInfo } from '../koala-swap.utils';

export async function getKoalaSwapAmmLiquidityQuote(
  network: string,
  poolAddress?: string,
  baseToken?: string,
  quoteToken?: string,
  ...
  if (!poolAddressToUse) {
    poolAddressToUse = await koalaSwap.findDefaultPool(baseToken, quoteToken, 'amm');
  ...
  if (existingPool) {
    ...
    const pairContract = new Contract(poolAddressToUse, IKoalaSwapV2PairABI.abi, ethereum.provider);
...
export const quoteLiquidityRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get liquidity quote for a KoalaSwap V2 pool',
    ...
    async (request) => {
      try {
        ...
        const poolInfo = await getKoalaSwapPoolInfo(poolAddress, network, 'amm');
        if (!poolInfo) {
