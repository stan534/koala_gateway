...
async function quoteAmmSwap(
  ...
  try {
    // Get the V2 pair
    const pair = await koalaSwap.getV2Pool(baseToken, quoteToken, poolAddress);
    if (!pair) {
    ...
    // Check for insufficient reserves error from KoalaSwap SDK
...
export async function getKoalaSwapAmmQuote(
  ...
  // Resolve tokens
  const baseTokenObj = koalaSwap.getTokenBySymbol(baseToken);
  const quoteTokenObj = koalaSwap.getTokenBySymbol(quoteToken);
...
async function formatSwapQuote(
  ...
  try {
    ...
    const { quote, koalaSwap, ethereum, baseTokenObj, quoteTokenObj } = await getKoalaSwapAmmQuote(
      fastify,
      network,
      poolAddress,
      baseToken,
...
export const quoteSwapRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get swap quote for KoalaSwap V2 AMM',
    ...
    async (request) => {
      try {
        ...
        if (poolAddressToUse) {
          ...
          const poolInfo = await getKoalaSwapPoolInfo(poolAddressToUse, networkToUse, 'amm');
          if (!poolInfo) {
          ...
          if (baseToken === poolInfo.baseTokenAddress) {
            ...
            const resolvedToken = koalaSwap.getTokenBySymbol(baseToken);
          ...
          poolAddressToUse = await koalaSwap.findDefaultPool(baseTokenToUse, quoteTokenToUse, 'amm');
