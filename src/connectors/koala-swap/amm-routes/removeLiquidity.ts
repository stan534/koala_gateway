...
import { getKoalaSwapV2RouterAddress, IKoalaSwapV2Router02ABI, IKoalaSwapV2PairABI } from '../koala-swap.contracts';
import { formatTokenAmount, getKoalaSwapPoolInfo } from '../koala-swap.utils';
import { KoalaSwapAmmRemoveLiquidityRequest } from '../schemas';
...
export const removeLiquidityRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{
    Body: Static<typeof KoalaSwapAmmRemoveLiquidityRequest>;
    Reply: RemoveLiquidityResponseType;
  }>(
    '/remove-liquidity',
    {
      schema: {
        description: 'Remove liquidity from a KoalaSwap V2 pool',
        tags: ['/connector/koala-swap'],
        body: KoalaSwapAmmRemoveLiquidityRequest,
        response: {
    ...
    async (request) => {
      try {
        ...
        // Get KoalaSwap and Ethereum instances
        ...
        if (!walletAddress) {
          walletAddress = await koalaSwap.getFirstWalletAddress();
          if (!walletAddress) {
        ...
        const baseTokenObj = koalaSwap.getTokenByAddress(poolInfo.baseTokenAddress);
        const quoteTokenObj = koalaSwap.getTokenByAddress(poolInfo.quoteTokenAddress);
        ...
        const pairContract = new Contract(poolAddress, IKoalaSwapV2PairABI.abi, wallet);
        ...
        const routerAddress = getKoalaSwapV2RouterAddress(networkToUse);
        const router = new Contract(routerAddress, IKoalaSwapV2Router02ABI.abi, wallet);
