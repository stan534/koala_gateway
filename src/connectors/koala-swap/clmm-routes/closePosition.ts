...
import { POSITION_MANAGER_ABI, getKoalaSwapV3NftManagerAddress } from '../koala-swap.contracts';
...
export async function closePosition(
  ...
  // Check NFT ownership
  try {
    await koalaSwap.checkNFTOwnership(positionAddress, walletAddress);
  } catch (error: any) {
  ...
  // Get the pool
  const pool = await koalaSwap.getV3Pool(token0, token1, position.fee);
  if (!pool) {
...
export const closePositionRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    {
      schema: {
        description: 'Close a KoalaSwap V3 position by removing all liquidity and collecting fees',
