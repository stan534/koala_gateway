...
import { getKoalaSwapV3SwapRouterAddress, ISwapRouterABI } from '../koala-swap.contracts';
import { formatTokenAmount } from '../koala-swap.utils';
import { KoalaSwapExecuteSwapRequest } from '../schemas';

import { getKoalaSwapClmmQuote } from './quoteSwap';
...
export async function executeClmmSwap(
  ...
  // Find pool address
  const poolAddress = await koalaSwap.findDefaultPool(baseToken, quoteToken, 'clmm');
  if (!poolAddress) {
  ...
  const { quote } = await getKoalaSwapClmmQuote(
    fastify,
    network,
    poolAddress,
    baseToken,
  ...
  const routerAddress = getKoalaSwapV3SwapRouterAddress(network);
  ...
  if (currentAllowance.lt(amountNeeded)) {
    ...
    throw fastify.httpErrors.badRequest(
      `Insufficient allowance for ${quote.inputToken.symbol}. Please approve at least ${formatTokenAmount(amountNeeded, quote.inputToken.decimals)} ${quote.inputToken.symbol} (${quote.inputToken.address}) for the KoalaSwap SwapRouter02 (${routerAddress})`,
    );
  }
  ...
  try {
    if (isHardwareWallet) {
      ...
      const iface = new utils.Interface(ISwapRouterABI);
      let data;
      ...
      const routerContract = new Contract(routerAddress, ISwapRouterABI, wallet);
...
export const executeSwapRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    {
      schema: {
        description: 'Execute a swap on KoalaSwap V3 CLMM using SwapRouter02',
        tags: ['/connector/koala-swap'],
        body: KoalaSwapExecuteSwapRequest,
    ...
    async (request) => {
      try {
        const { walletAddress, network, baseToken, quoteToken, amount, side, slippagePct } =
          request.body as typeof KoalaSwapExecuteSwapRequest._type;
