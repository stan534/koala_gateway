...
import { getKoalaSwapV3NftManagerAddress } from '../koala-swap.contracts';
...
export async function openPosition(
  ...
  const poolInfo = await getKoalaSwapPoolInfo(poolAddress, network, 'clmm');
  if (!poolInfo) {
  ...
  // Get the V3 pool
  const pool = await koalaSwap.getV3Pool(baseTokenObj, quoteTokenObj, undefined, poolAddress);
  if (!pool) {
...
export const openPositionRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{
    ...
    {
      schema: {
        description: 'Open a new liquidity position in a KoalaSwap V3 pool',
