...
export async function getPositionInfo(
  ...
  const pool = await koalaSwap.getV3Pool(token0, token1, fee);
  if (!pool) {
  ...
  const poolAddress = computePoolAddress({
    factoryAddress: getKoalaSwapV3FactoryAddress(network),
    tokenA: token0,
    tokenB: token1,
    fee,
  });
...
export const positionInfoRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get position information for a KoalaSwap V3 position',
