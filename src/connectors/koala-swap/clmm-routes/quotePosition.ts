...
import { parseFeeTier, getKoalaSwapPoolInfo } from '../koala-swap.utils';
...
export const quotePositionRoute: FastifyPluginAsync = async (fastify) => {
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get a quote for opening a position on KoalaSwap V3',
    ...
    async (request) => {
      try {
        ...
        const poolInfo = await getKoalaSwapPoolInfo(poolAddress, networkToUse, 'clmm');
        if (!poolInfo) {
        ...
        // In KoalaSwap, ticks are log base 1.0001 of price
        ...
        //
        // The issue is that KoalaSwap stores the price and tick in a specific way:
