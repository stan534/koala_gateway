...
import { formatTokenAmount, parseFeeTier, getKoalaSwapPoolInfo } from '../koala-swap.utils';
...
async function quoteClmmSwap(
  ...
  try {
    ...
    const pool = await koalaSwap.getV3Pool(
      baseToken,
      quoteToken,
...
export async function getKoalaSwapClmmQuote(
...
async function formatSwapQuote(
  ...
  try {
    ...
    const { quote, koalaSwap, ethereum, baseTokenObj, quoteTokenObj } = await getKoalaSwapClmmQuote(
      fastify,
      network,
      poolAddress,
      baseToken,
...
export const quoteSwapRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get swap quote for KoalaSwap V3 CLMM',
    ...
    async (request) => {
      try {
        ...
        if (poolAddressToUse) {
          ...
          const poolInfo = await getKoalaSwapPoolInfo(poolAddressToUse, networkToUse, 'clmm');
          if (!poolInfo) {
          ...
          poolAddressToUse = await koalaSwap.findDefaultPool(baseTokenToUse, quoteTokenToUse, 'clmm');
