...
import {
  IKoalaSwapV3FactoryABI,
  ISwapRouterABI,
  IKoalaSwapV2Router02ABI,
  IKoalaSwapV2PairABI,
  getKoalaSwapV2RouterAddress,
  getKoalaSwapV2FactoryAddress,
  getKoalaSwapV3SwapRouterAddress,
  getKoalaSwapV3NftManagerAddress,
  getKoalaSwapV3QuoterV2ContractAddress,
  getKoalaSwapV3FactoryAddress,
} from './koala-swap.contracts';
import { isValidV3Pool, isValidV2Pool } from './koala-swap.utils';

export class KoalaSwap {
  private static _instances: { [name: string]: KoalaSwap };

  // Ethereum chain instance
  private ethereum: Ethereum;

  // Configuration
  public config: KoalaSwapConfig.RootConfig;
  ...
  private constructor(network: string) {
    ...
    this.config = KoalaSwapConfig.config;
  }

  public static async getInstance(network: string): Promise<KoalaSwap> {
    if (KoalaSwap._instances === undefined) {
      KoalaSwap._instances = {};
    }

    if (!(network in KoalaSwap._instances)) {
      KoalaSwap._instances[network] = new KoalaSwap(network);
      await KoalaSwap._instances[network].init();
    }

    return KoalaSwap._instances[network];
  }

  /**
   * Initialize the Koala Swap instance
   */
  public async init() {
    try {
      ...
      this.v2Factory = new Contract(
        getKoalaSwapV2FactoryAddress(this.networkName),
        IKoalaSwapV3FactoryABI.abi, // V2 factory uses same interface as V3
        this.ethereum.provider,
      );

      // Initialize V3 (CLMM) contracts
      this.v3Factory = new Contract(
        getKoalaSwapV3FactoryAddress(this.networkName),
        IKoalaSwapV3FactoryABI.abi,
        this.ethereum.provider,
      );

      // Initialize NFT Manager with minimal ABI
      this.v3NFTManager = new Contract(
        getKoalaSwapV3NftManagerAddress(this.networkName),
        [
          {
      ...
      this.v3Quoter = new Contract(
        getKoalaSwapV3QuoterV2ContractAddress(this.networkName),
        [
          {
            inputs: [
      ...
      // Initialize SwapRouter for router operations
      this.v3SwapRouter = new Contract(
        getKoalaSwapV3SwapRouterAddress(this.networkName),
        ISwapRouterABI,
        this.ethereum.provider,
      );
      ...
      this._ready = true;
      logger.info(`Koala Swap connector initialized for network: ${this.networkName}`);
    } catch (error) {
      logger.error(`Error initializing Koala Swap: ${error.message}`);
      throw error;
    }
  }

  /**
   * Check if the Koala Swap instance is ready
   */
  ...
  public getTokenByAddress(address: string): Token | null {
    ...
    // Create Uniswap SDK Token instance (Koala Swap uses Uniswap V3 SDK)
   ...
   */
  public getKoalaSwapToken(tokenInfo: TokenInfo): Token {
  ...
  public async getV2Pool(tokenA: Token | string, tokenB: Token | string, poolAddress?: string): Promise<V2Pair | null> {
    try {
      ...
      const pairContract = new Contract(pairAddress, IKoalaSwapV2PairABI.abi, this.ethereum.provider);
  ...
  /**
   * Find a default pool for a token pair in AMM or CLMM
   */
  public async findDefaultPool(
    ...
    try {
      ...
      if (!pool) {
        logger.warn(
          `No ${poolType} pool found for ${baseTokenInfo.symbol}-${quoteTokenInfo.symbol} on Koala Swap network ${this.networkName}`,
        );
        return null;
      }
  ...
  /**
   * Check NFT ownership for Koala Swap V3 positions
  ...
  /**
   * Check NFT approval for Koala Swap V3 positions
   * @param positionId The NFT position ID
  ...
  public async checkNFTApproval(positionId: string, walletAddress: string, operatorAddress: string): Promise<void> {
    ...
    if (approvedAddress.toLowerCase() !== operatorAddress.toLowerCase() && !isApprovedForAll) {
      throw new Error(
        `Insufficient NFT approval. Please approve the position NFT (${positionId}) for the Koala Swap Position Manager (${operatorAddress})`,
      );
    }
  }

  /**
   * Get V3 Factory contract
   */
  public getV3Factory(): Contract {
    return this.v3Factory;
  }

  /**
   * Get V3 NFT Manager contract
   */
  public getV3NFTManager(): Contract {
    return this.v3NFTManager;
  }

  /**
   * Get V3 Quoter contract
   */
  public getV3Quoter(): Contract {
    return this.v3Quoter;
  }

  /**
   * Get V3 SwapRouter contract
   */
  public getV3SwapRouter(): Contract {
    return this.v3SwapRouter;
  }
  ...
  /**
   * Close the Koala Swap instance and clean up resources
   */
  public async close() {
    // Clean up resources
    if (this.networkName in KoalaSwap._instances) {
      delete KoalaSwap._instances[this.networkName];
    }
  }
}
