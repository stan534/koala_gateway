...
/**
 * Determine if a pool address is a valid Koala Swap V3 pool
 * @param poolAddress The pool address to check
 * @returns True if the address is a valid V3 pool, false otherwise
 */
 ...
 * This helper function is used by the CLMM routes
...
/**
 * Creates a Koala Swap V3 Pool instance with a tick data provider
 ...
 */
export function getKoalaSwapV3PoolWithTickProvider(
  tokenA: Token,
  tokenB: Token,
  fee: FeeAmount,
  sqrtPriceX96: string,
...
/**
 * Pool info interface for Koala Swap pools
 */
export interface KoalaSwapPoolInfo {
...
/**
 * Get pool information for a Koala Swap V2 (AMM) pool
 * @param poolAddress The pool address
 * @param network The network name
 * @returns Pool information with base and quote token addresses
 */
export async function getV2PoolInfo(poolAddress: string, network: string): Promise<KoalaSwapPoolInfo | null> {
  try {
    const ethereum = await Ethereum.getInstance(network);
    const { IKoalaSwapV2PairABI } = await import('./koala-swap.contracts');

    // Create pair contract
    const pairContract = new Contract(poolAddress, IKoalaSwapV2PairABI.abi, ethereum.provider);
...
/**
 * Get pool information for a Koala Swap V3 (CLMM) pool
 * @param poolAddress The pool address
 * @param network The network name
 * @returns Pool information with base and quote token addresses
 */
export async function getV3PoolInfo(poolAddress: string, network: string): Promise<KoalaSwapPoolInfo | null> {
  try {
    const ethereum = await Ethereum.getInstance(network);
...
/**
 * Get pool information for any Koala Swap pool (V2 or V3)
 * @param poolAddress The pool address
 * @param network The network name
 * @param poolType Optional pool type hint
 * @returns Pool information with base and quote token addresses
 */
export async function getKoalaSwapPoolInfo(
  poolAddress: string,
  network: string,
  poolType?: 'amm' | 'clmm',
): Promise<KoalaSwapPoolInfo | null> {
...
/**
 * Check if a pool address is a valid Koala Swap V2 pool
 * @param poolAddress The pool address to check
 * @returns True if the address is a valid V2 pool, false otherwise
 */
