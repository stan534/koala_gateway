...
import { KoalaSwapExecuteQuoteRequest } from '../schemas';
...
async function executeQuote(
    ...
    if (BigNumber.from(tokenToPermit2Allowance).lt(requiredAllowance)) {
      ...
      throw fastify.httpErrors.badRequest(
        `Insufficient ${inputToken.symbol} allowance to Permit2. ` +
          ...
          `Please approve ${inputToken.symbol} using spender: "koala-swap/router"`,
      );
    }
      ...
      if (isExpired) {
        ...
        throw fastify.httpErrors.badRequest(
          `Permit2 allowance for ${inputToken.symbol} to Universal Router has expired. ` +
            ...
            `Please approve ${inputToken.symbol} again using spender: "koala-swap/router"`,
        );
      } else {
        throw fastify.httpErrors.badRequest(
  ...
  try {
    if (isHardwareWallet) {
      ...
      // Get gas options with increased gas limit for Universal Router V2
      // KoalaSwap Universal Router V2 swaps typically use between 200k-500k gas
      const gasLimit = 500000; // Increased for Universal Router V2
      const gasOptions = await ethereum.prepareGasOptions(undefined, gasLimit);
    ...
    if (errorSelector === '0xd81b2f2e') {
      ...
      throw fastify.httpErrors.badRequest(
        `Universal Router error: Permit2 allowance has expired for ${inputToken.symbol}. ` +
          `Please re-approve the token using spender: "koala-swap/router" to set a new expiration.`,
      );
...
export const executeQuoteRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    {
      schema: {
        description: 'Execute a previously fetched quote from KoalaSwap Universal Router',
        tags: ['/connector/koala-swap'],
        body: KoalaSwapExecuteQuoteRequest,
    ...
    async (request) => {
      try {
        const {
          ...
          quoteId,
        } = request.body as typeof KoalaSwapExecuteQuoteRequest._type;
