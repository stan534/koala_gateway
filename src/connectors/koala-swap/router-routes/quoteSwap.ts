...
import { KoalaSwapQuoteSwapRequest, KoalaSwapQuoteSwapResponse } from '../schemas';
...
async function quoteSwap(
  ...
  side: 'BUY' | 'SELL',
  slippagePct: number,
): Promise<Static<typeof KoalaSwapQuoteSwapResponse>> {
  ...
  const koalaSwap = await KoalaSwap.getInstance(network);
  ...
  // Convert to KoalaSwap SDK Token objects
  const baseTokenObj = koalaSwap.getKoalaSwapToken(baseTokenInfo);
  const quoteTokenObj = koalaSwap.getKoalaSwapToken(quoteTokenInfo);
  ...
  // TODO: Implement router quote functionality for Koala Swap
  // Koala Swap doesn't have a Universal Router like Uniswap
  // For now, this route is not functional
  throw fastify.httpErrors.notImplemented('Router routes are not yet implemented for Koala Swap');

  /* Commented out until router functionality is implemented
  // Get quote from Universal Router
  logger.info(`[quoteSwap] Calling getUniversalRouterQuote...`);
  const quoteResult = await koalaSwap.getUniversalRouterQuote(inputToken, outputToken, amount, side, walletAddress);
  ...
  // Store both quote and request data in the quote object for KoalaSwap
  const cachedQuote = {
    quote: {
      ...quoteResult,
...
export const quoteSwapRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    Reply: Static<typeof KoalaSwapQuoteSwapResponse>;
  }>(
    '/quote-swap',
    {
      schema: {
        description: 'Get an executable swap quote from KoalaSwap Universal Router',
        tags: ['/connector/koala-swap'],
        querystring: KoalaSwapQuoteSwapRequest,
        response: { 200: KoalaSwapQuoteSwapResponse },
      },
    },
    async (request) => {
      try {
        const {
          ...
          amount,
          side,
          slippagePct = KoalaSwapConfig.config.slippagePct,
        } = request.query as typeof KoalaSwapQuoteSwapRequest._type;
